...
export default function ProfileModal({ isOpen, onClose }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await apiService.updateProfile(account, formData)
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="My Profile">
      <div style={{ textAlign: 'center', marginBottom: '24px' }}>
        ...
        <span className="tag" style={{ textTransform: 'capitalize', fontSize: '0.875rem' }}>
          {userProfile.role}
        </span>
      </div>
      ...
      {editing ? (
        <div className="flex-col" style={{ gap: '12px' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '4px', fontSize: '0.875rem', color: 'var(--text-muted)' }}>
              Full Name *
            </label>
            <input
          ...
          <div className="quick-actions" style={{ marginTop: '8px' }}>
            <Button onClick={() => setEditing(false)} ghost>Cancel</Button>
        ...
        <>
          <div className="detail-row">
            ...
            <span className="detail-value">{userProfile.name || 'Not set'}</span>
          </div>
          ...
          {userProfile.role === 'doctor' && (
            <>
              <div className="detail-row">
                ...
                <span className="detail-value">{userProfile.hospital || 'Not set'}</span>
              </div>
              
              <div className="detail-row">
                <span className="detail-label">Specialty</span>
                <span className="detail-value">{userProfile.specialty || 'Not set'}</span>
              </div>
            </>
          )}
          ...
          <details style={{ marginTop: '16px', fontSize: '0.875rem' }}>
            ...
            <div style={{ 
              ...
              <div style={{ marginTop: '8px', display: 'flex', gap: '8px' }}>
                ...
                <Button 
                  onClick={() => window.open(`https://sepolia.etherscan.io/address/${userProfile.walletAddress}`, '_blank')} 
                  ghost 
                  style={{ padding: '6px 12px', fontSize: '0.75rem' }}
                >
                  Etherscan
