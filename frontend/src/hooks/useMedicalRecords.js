...
export function useMedicalRecords() {
  ...
  const getAuthorizedDoctors = useCallback(async (patientAddress) => {
    if (!contract) {
...
const enrichedRecords = await Promise.all(
  records.map(async (record, idx) => {
    try {
      ...
      try {
        const backendData = await apiService.getRecordByCid(record.ipfsCID)
        ...
        return {
          ...
          uploadedByName: backendData?.doctor_address || record.uploadedBy,
          txHash: backendData?.tx_hash || null,
          record_type: backendData?.record_type,
        ...
        if (backendErr.message?.includes('404')) {
          console.info('Record not in backend (blockchain only):', record.ipfsCID)
        } else {
          console.warn('Backend enrichment failed:', backendErr.message)
        }
        
        // Return blockchain data only
        return {
          ...record,
          id: idx,
          filename: `Record-${record.ipfsCID.slice(-6)}`,
        }
      }
    } catch (err) {
  ...
  const uploadRecord = useCallback(async (patientAddress, file, recordType, description) => {
    if (!contract || !account) {
    ...
    try {
      ...
      const hasAccess = await contract.hasAccess(patientAddress, account)
      if (!hasAccess) {
        throw new Error('Doctor does not have access to upload for this patient')
      }
      ...
      const encryptedBytes = Uint8Array.from(atob(encryptionResult.encrypted_content), c => c.charCodeAt(0))
      ...
      const cid = await ipfsService.uploadFile(encryptedFile, `${file.name}.encrypted`)
      ...
      logger.info('Backend save complete, record ID:', backendResult?.record_id)

      addLog(`Record uploaded successfully! CID: ${cid.slice(0, 20)}...`, 'info')
      return {
        success: true,
        cid,
        txHash: tx.hash,
  ...
  const downloadAndDecrypt = useCallback(async (cid, iv, key, expectedHash, filename) => {
    ...
    try {
      ...
      const decryptedResult = await encryptionService.decryptFile(base64, iv, key)
      logger.info('Decryption complete, size:', decryptedResult.size)

      // Step 4: Verify hash
      logger.info('Step 4: Verifying integrity...')
      const decryptedBytes = Uint8Array.from(atob(decryptedResult.content_base64), c => c.charCodeAt(0))
      const decryptedBlob = new Blob([decryptedBytes])
      const decryptedFile = new File([decryptedBlob], filename)
      
      const verification = await encryptionService.verifyFile(decryptedFile, expectedHash)
      if (!verification.verified) {
        logger.error('File verification failed!')
        addLog('WARNING: File integrity check failed!', 'error')
        throw new Error('File integrity verification failed - possible tampering')
      }
